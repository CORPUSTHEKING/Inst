...
> If deleting and recreating it causes no runtime breakage, symlinking is permitted.

---

## B3. ❌ Files That Must NEVER Be Moved or Symlinked to Shared
...
> If it contains binaries, sockets, symlinks, or assumes POSIX semantics, it must remain in `$HOME` or `$PREFIX`.
...
- Repair Flutter in `$HOME` or `$PREFIX`
- Rebuild **only** permitted symlinks
- Freeze filesystem topology

Automation is permitted **only with explicit approval**, extensive validation, and precise calibration.